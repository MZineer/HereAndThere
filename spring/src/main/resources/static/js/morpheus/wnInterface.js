...
var wnIf = new function (a) {
    this.init = function () {
        ...
        if (window.Web !== a) {
            ...
            window.VirtualInterface = Web.instance.virtualInterfaceWithContext(window)
        } else {
            if (parent !== a && parent.Emulator !== a) {
                console.log("parent.Emulatore");
                this.device = DT_VIRTUAL;
                window.VirtualInterface = parent.Emulator.sharedInstance().currentProject().virtualInterfaceWithContext(window)
            } else {
...
function DataSendingHandler() {
    ...
    this.putString = function (b, a) {
        if (a > 0) {
            this.data.dataArray[this.ptr++] = getRequestDataObject(DataSendingHandler.TYPE_STRING, b.cutByte(a), a);
            this.data.length += a
        }
    };
    this.putInt = function (a) {
        this.data.dataArray[this.ptr++] = getRequestDataObject(DataSendingHandler.TYPE_INT, a, 4);
        this.data.length += 4
    };
    this.putShort = function (a) {
        this.data.dataArray[this.ptr++] = getRequestDataObject(DataSendingHandler.TYPE_SHORT, a, 2);
        this.data.length += 2
    };
    this.putByte = function (a) {
        this.data.dataArray[this.ptr++] = getRequestDataObject(DataSendingHandler.TYPE_BYTE, a, 1);
        this.data.length += 1
    };
...
function WNResetAllStorageVariables() {
    if (wnIf.device === DT_IOS) {
        ...
        if (wnIf.device === DT_ANDROID) {
            ...
            if (wnIf.device === DT_VIRTUAL) {
                return JSON.parse(VirtualInterface.execute("wnResetAllStorageVariables"))
            }
        }
    }
}
...
function WNRequestJsonDataToServer(f, c, j, i, p, g) {
    ...
    g = (g === undefined) ? "" : g;
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNRequestJsonDataToServer", f, c, j, JSON.stringify(i), l, n, o, e, m, b, g, k, JSON.stringify(a), d)
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnRequestJsonDataToServer", f, c, checkCallBackParam(j), JSON.stringify(i), l, n, o, e, m, b, g, k, JSON.stringify(a), d, document.location.href)
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnRequestJsonDataToServer", f, c, j, JSON.stringify(i), l, n, o, e, m, b, g, k, JSON.stringify(a), d)
            }
        }
    }
}
...
function WNRequestSocketDataToServer(e, c, h, n, g, m, f) {
    ...
    f = (f === undefined) ? "" : f;
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNRequestSocketDataToServer", e, c, h, JSON.stringify(n), JSON.stringify(g), i, k, l, b, f, JSON.stringify(a))
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnRequestSocketDataToServer", e, c, checkCallBackParam(h), JSON.stringify(n), JSON.stringify(g), i, k, l, d, j, b, f, JSON.stringify(a))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execure("wnRequestSocketDataToServer", e, c, h, JSON.stringify(n), JSON.stringify(g), i, k, l, b, f, JSON.stringify(a))
            }
        }
    }
}
...
function WNResponseJsonDataFromServer(trCode, callBackFuncName, recvData, tagId, userData) {
    var callback;
    try {
        callback = eval(callBackFuncName)
    } catch (e) {
        console.error("WNResponseJsonDataFromServer", "callback 함수를 찾을 수 없습니다.", callBackFuncName);
        return
    }
    if (typeof recvData == "string") {
        var trimRecvData = recvData.replace(/\↵/, "\\n").replace(/\n/g, "\\n").replace(/\\n/g, "\\n").replace(/\\'/g, "\\'").replace(/\\"/g, '\\"').replace(/\\&/g, "\\&").replace(/\\r/g, "\\r").replace(/\\t/g, "\\t").replace(/\\b/g, "\\b").replace(/\\f/g, "\\f");
...
function WNResponseSocketDataFromServer(trCode, callBackFuncName, recvData, tagId, userData) {
    try {
        if (wnIf.device === DT_IOS) {
            eval(callBackFuncName)(trCode, JSON.parse(recvData), tagId, JSON.parse(userData))
        } else {
            if (wnIf.device === DT_ANDROID) {
                eval(callBackFuncName)(trCode, JSON.parse(recvData), tagId, JSON.parse(userData))
            } else {
...
function WNMoveToWebOnNewAct(d, c, e, b) {
    ...
    var a = getUrlInfo(d);
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNMoveToWebOnNewAct", a.fileName, a.parameters, c, e, b)
    } else {
        if (wnIf.device === DT_ANDROID) {
            Native.wnRequestMoveToWebOnActivity(a.fileName, encodeURIComponent(a.parameters), c, e, b)
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnMoveToWebOnNewAct", a.fileName, a.parameters, c, e, b)
            }
        }
    }
}
...
function WNRequestDatePicker(c, f, d, e, g, a) {
    ...
    if (f === "HM12") {
        ...
        if (f === "HM24") {
            ...
            if (f === "YMD" || f === "DMY" || f === "YM" || f === "MMYYYY" || f === "YYYY" || f === "MM" || f === "DD") {
                var h = 4;
                if (f === "YMD" || f === "DMY") {
                    h = 8
                } else {
    ...
    if (wnIf.device === DT_IOS) {
        ...
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnRequestDatePicker", checkCallBackParam(c), f, d, b, i, a)
        } else {
            if (wnIf.device === DT_VIRTUAL) {
...
function WNHttpFileUpload(b, i, d, g, c, h, a) {
    ...
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNRequestHttpFileTrans", b, i, JSON.stringify(d), (g) ? "TRUE" : "FALSE", (c) ? "TRUE" : "FALSE", j, f)
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnRequestHttpFileTrans", b, j, i, JSON.stringify(d), (g) ? "TRUE" : "FALSE", (c) ? "TRUE" : "FALSE", f)
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnRequestHttpFileTrans", b, i, JSON.stringify(d), (g) ? "TRUE" : "FALSE", (c) ? "TRUE" : "FALSE", j, f)
            }
        }
    }
}
...
function WNFtpFileUpload(a, b, c) {
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNFtpFileUpload", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnRequestFtpFileTrans", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnFtpFileUpload", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
            }
        }
    }
}
...
function WNNormalChoiceListPopup(e, d, a, b) {
    var c = (b === true) ? "Y" : "N";
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNNormalChoiceListPopup", e, JSON.stringify(d), a, c)
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnNormalListPopup", e, JSON.stringify(d), checkCallBackParam(a), c)
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnNormalChoiceListPopup", e, JSON.stringify(d), a, c)
            }
        }
    }
}
...
function WNSingleChoicePopup(h, g, d, a) {
    ...
    var c = (a === true) ? "Y" : "N";
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNSingleChoicePopup", h, JSON.stringify(g), JSON.stringify(d), c)
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnSingleChoiceListPopup", h, JSON.stringify(g), JSON.stringify(d), c)
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnSingleChoiceListPopup", h, JSON.stringify(g), JSON.stringify(d), c)
            }
        }
    }
}
...
function WNMultiChoicePopup(h, g, d, a) {
    ...
    var c = (a === true) ? "Y" : "N";
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNMultiChoicePopup", h, JSON.stringify(g), JSON.stringify(d), c)
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnMultiChoiceListPopup", h, JSON.stringify(g), JSON.stringify(d), c)
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnMultiChoiceListPopup", h, JSON.stringify(g), JSON.stringify(d), c)
            }
        }
    }
}
...
function WNRemoveMediaFiles(b, c, e, a) {
    ...
    if (wnIf.device === DT_IOS) {
        ...
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnRemoveMediaFilesInfo", JSON.stringify(b), c, checkCallBackParam(e), d)
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnRemoveMediaFilesInfo", JSON.stringify(b), c, e, d)
            }
        }
    }
}
...
function WNFileIoCreate(a) {
    ...
    if (wnIf.device === DT_IOS) {
        return JSON.parse(IOSNative.callSynchronous("WNFileIoCreate", JSON.stringify(a)))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return JSON.parse(WN2Common("wnFileIoCreate", JSON.stringify(a)))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return JSON.parse(VirtualInterface.execute("wnFileIoCreate", JSON.stringify(a)))
            }
        }
    }
}
...
function WNFileIoDelete(a) {
    ...
    if (wnIf.device === DT_IOS) {
        return JSON.parse(IOSNative.callSynchronous("WNFileIoDelete", JSON.stringify(a)))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return JSON.parse(WN2Common("wnFileIoDelete", JSON.stringify(a)))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return JSON.parse(VirtualInterface.execute("wnFileIoDelete", JSON.stringify(a)))
            }
        }
    }
}
...
function WNFileIoWrite(a) {
    ...
    if (wnIf.device === DT_IOS) {
        return JSON.parse(IOSNative.callSynchronous("WNFileIoWriteFile", JSON.stringify(a)))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return JSON.parse(WN2Common("wnFileIoWriteFile", JSON.stringify(a)))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return JSON.parse(VirtualInterface.execute("wnFileIoWriteFile", JSON.stringify(a)))
            }
        }
    }
}
...
function WNFileIoCopy(a) {
    if (a.source === undefined || a.destination === undefined || a.overwrite === undefined || a.option === undefined) {
        var b = {};
        b.status = "FILEIO INPUT PARAM ERROR";
        return b
    }
    if (wnIf.device === DT_IOS) {
        return JSON.parse(IOSNative.callSynchronous("WNFileIoCopy", JSON.stringify(a)))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return JSON.parse(WN2Common("wnFileIoCopy", JSON.stringify(a)))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return JSON.parse(VirtualInterface.execute("wnFileIoCopy", JSON.stringify(a)))
            }
        }
    }
}
...
function WNFileIoMove(a) {
    ...
    if (wnIf.device === DT_IOS) {
        return JSON.parse(IOSNative.callSynchronous("WNFileIoMove", JSON.stringify(a)))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return JSON.parse(WN2Common("wnFileIoMove", JSON.stringify(a)))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return JSON.parse(VirtualInterface.execute("wnFileIoMove", JSON.stringify(a)))
            }
        }
    }
}
...
function WNFileIoInfo(a) {
    ...
    if (wnIf.device === DT_IOS) {
        return JSON.parse(IOSNative.callSynchronous("WNFileIoInfo", JSON.stringify(a)))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return JSON.parse(WN2Common("wnFileIoInfo", JSON.stringify(a)))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return JSON.parse(VirtualInterface.execute("wnFileIoInfo", JSON.stringify(a)))
            }
        }
    }
}
...
function WNFileIoList(a) {
    ...
    if (wnIf.device === DT_IOS) {
        return JSON.parse(IOSNative.callSynchronous("WNFileIoList", JSON.stringify(a)))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return JSON.parse(WN2Common("wnFileIoList", JSON.stringify(a)))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return JSON.parse(VirtualInterface.execute("wnFileIoList", JSON.stringify(a)))
            }
        }
    }
}
...
function WNLocalDbCreate(db_name, cb_func) {
    var return_value;
    if (wnIf.device === DT_IOS) {
        return_value = JSON.parse(IOSNative.callSynchronous("WNLocalDbCreate", db_name))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return_value = JSON.parse(WN2Common("wnLocalDbCreate", db_name))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return_value = JSON.parse(VirtualInterface.execute("wnLocalDbCreate", db_name))
            }
        }
    }
...
function WNLocalDbDelete(db_name, cb_func) {
    var return_value;
    if (wnIf.device === DT_IOS) {
        return_value = JSON.parse(IOSNative.callSynchronous("WNLocalDbDelete", db_name))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return_value = JSON.parse(WN2Common("wnLocalDbDelete", db_name))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return_value = JSON.parse(VirtualInterface.execute("wnLocalDbDelete", db_name))
            }
        }
    }
    return_value.db_name = db_name;
...
function WNLocalDbOpen(db_name, cb_func) {
    var return_value;
    if (wnIf.device === DT_IOS) {
        return_value = JSON.parse(IOSNative.callSynchronous("WNLocalDbOpen", db_name))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return_value = JSON.parse(WN2Common("wnLocalDbOpen", db_name))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return_value = JSON.parse(VirtualInterface.execute("wnLocalDbOpen", db_name))
            }
        }
    }
    return_value.db_name = db_name;
...
function WNLocalDbClose(db_name, cb_func) {
    var return_value;
    if (wnIf.device === DT_IOS) {
        return_value = JSON.parse(IOSNative.callSynchronous("WNLocalDbClose", db_name))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return_value = JSON.parse(WN2Common("wnLocalDbClose", db_name))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return_value = JSON.parse(VirtualInterface.execute("wnLocalDbClose", db_name))
            }
        }
    }
    return_value.db_name = db_name;
...
function WNLocalDbExecuteSql(input_data, cb_func) {
    ...
    if (wnIf.device === DT_IOS) {
        return_value = JSON.parse(IOSNative.callSynchronous("WNLocalDbExecuteSql", JSON.stringify(input_data)))
    } else {
        if (wnIf.device === DT_ANDROID) {
            return_value = JSON.parse(WN2Common("wnLocalDbExecuteSql", JSON.stringify(input_data)))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return_value = JSON.parse(VirtualInterface.execute("wnLocalDbExecuteSql", JSON.stringify(input_data)))
            }
        }
    }
...
function WNFtpFileDownload(a, b, c) {
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNFtpFileDownload", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnRequestFtpFileDownload", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnFtpFileDownload", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
            }
        }
    }
}
...
function WNFtpListDownload(a, b, c) {
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNFtpListDownload", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnRequestFtpFileDownloadListinfo", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnFtpListDownload", JSON.stringify(a), JSON.stringify(b), (c) ? "TRUE" : "FALSE")
            }
        }
    }
}
...
function WNTakeQRCode(b, c) {
    ...
    if (wnIf.device === DT_IOS) {
        ...
        if (wnIf.device === DT_ANDROID) {
            return WN2Common("wnTakeQRCode", checkCallBackParam(b), JSON.stringify(a))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                return VirtualInterface.execute("wnTakeQRCode", checkCallBackParam(b), JSON.stringify(a))
            }
        }
    }
}
...
function WNZip(b, d, c, h) {
    ...
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNZip", b, JSON.stringify(d), c, JSON.stringify(h));
        return
    } else {
        if (wnIf.device === DT_ANDROID) {
...
function WNHttpLoginUpdateResources(a, b) {
    ...
    if (wnIf.device === DT_IOS) {
        IOSNative.callAsynchronous("WNLoginUpdateResourceFiles", a, JSON.stringify(b))
    } else {
        if (wnIf.device === DT_ANDROID) {
            WN2CommonAsync("wnLoginUpdateResources", a, JSON.stringify(b))
        } else {
            if (wnIf.device === DT_VIRTUAL) {
                VirtualInterface.execute("wnLoginUpdateResources", a, JSON.stringify(b))
            }
        }
    }
}
